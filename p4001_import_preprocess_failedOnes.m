% 01/23/2021 Makoto. GoodIcIdx cannot be all the ICs. Stupid. (11/07/2020 Makoto)
% 01/21/2021 Makoto. Debugged.
...
% The FAILED files.
allFailed = dir('/data/mobi/Hiroki/p4000_import_preprocess_n600/FAILED_*.mat');
allFailedNames = cellfun(@(x) x(8:end-4), {allFailed.name}, 'uniformoutput', false);
allEdfNames    = cellfun(@(x) x(1:end-4), {allEdf.name}, 'uniformoutput', false);

% The difference.
[C,IA,IB] = intersect(allEdfNames, allFailedNames);

for edfIdx = 1:length(IA)
    
    % Import the .edf file.
    currentFailedIdx = IA(edfIdx);
    currentEdfName = allEdf(currentFailedIdx).name;
    ...
    if length(goodIcIdx)~=EEG.nbchan
        EEG = pop_subcomp(EEG, goodIcIdx, 0, 1); % I got error here; goodIcIdx cannot be all the ICs. Stupid. (11/07/2020 Makoto)
    end
    ...
    % Delete failure log.
    delete(sprintf('/data/mobi/Hiroki/p4000_import_preprocess_n600/FAILED_%s.mat', EEG.setname));
end
